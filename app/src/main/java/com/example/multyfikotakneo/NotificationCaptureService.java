...
public class NotificationCaptureService extends NotificationListenerService {
    ...
    public void onNotificationPosted(StatusBarNotification sbn) {
        ...
        // Multyfi groups several child notifications together. Skip the Android group-summary
        // notification so old entry + new exit texts are never parsed as one mixed signal.
        ...
        if (SignalParser.looksLikePreAlert(title, body.toString())) {
            ApprovalNotifier.showStatus(
                    this,
                    "Multyfi 15-minute trade alert detected",
                    "Trade is not released yet. Waiting for the 'Released: Equity Intraday Trade' notification.");
            return;
        }

        boolean exitUpdate = ExitAlertParser.looksLikeExit(title, body.toString());
        boolean entryAlert = SignalParser.looksLikeEntry(title, body.toString());
        if (!exitUpdate && !entryAlert) {
            ApprovalNotifier.showStatus(
                    this,
                    "Multyfi notification ignored",
                    "No released entry or exit instruction was detected, so no trade ticket was created.");
            return;
        }
    ...
    private void processEntry(String title, String body, SettingsRepo settings, TradeStore store) {
        try {
            ...
            // Quantity is intentionally NOT sized from a user-entered capital amount.
            // We prepare a 1-share placeholder so the approved ticket can reach Kotak's MIS order form.
            // Accessibility then reads Kotak's live Available Margin and Margin Required and calculates the final entry quantity there.
            TradeTicket ticket = TradeDecision.build(
                    signal, ltp, Math.max(ltp * 2.0, 1000.0), 100.0,
            ...
            ticket.reason = ticket.reason + " Entry quantity will be calculated from Kotak Neo's live MIS margin on the order page; manual capital is ignored.";
            store.saveTicket(ticket);
            store.appendLog("ENTRY_TICKET_PREPARED", ticket,
                    ticket.reason + " Kotak token: " + instrument.token + ", trading symbol: " + instrument.tradingSymbol + ".");
