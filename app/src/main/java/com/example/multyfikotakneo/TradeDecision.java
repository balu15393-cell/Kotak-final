...
public final class TradeDecision {
    ...
    public static TradeTicket build(Signal s, double ltp, double capital, double utilizationPct,
        ...
        if (s.target == null || s.target <= 0) throw new IllegalArgumentException("Multyfi target is missing or invalid.");
        if (s.stopLoss == null || s.stopLoss <= 0) throw new IllegalArgumentException("Multyfi stop-loss is missing or invalid.");
        ...
        t.quantity = quantity;
        t.capital = capital;
        // Multyfi owns both protective levels. Never recalculate or replace them.
        t.target = s.target;
