...
public final class ExitAlertParser {
    ...
    private static final Set<String> BLACKLIST = new HashSet<>(Arrays.asList(
            ...
            "PRICE", "RETURNS", "CAPITAL", "DURATION", "EQUITY", "RELEASED", "NEW", "TRADES",
            "RELEASING", "SOON", "EARLY", "AGAINST", "PLANNED", "SECURING", "GAINS"
    ));
    ...
    public static boolean looksLikeExit(String title, String body) {
        ...
        return containsAny(lower,
                ...
                "exit cost to cost", "exit at cost", "early exit", "disciplined early exit");
    }
    ...
    public static ExitAlert parse(String title, String body) {
        ...
        } else if (containsAny(lower, "early exit", "disciplined early exit")) {
            reason = "Multyfi requested a disciplined early exit before the planned target.";
        } else {
    ...
    private static String extractSymbol(String upper) {
        ...
        Pattern[] patterns = new Pattern[] {
                // Prefer the highly specific Multyfi title formats first.
                Pattern.compile("\\b" + token + "\\s+UPDATE\\b"),
                Pattern.compile("\\b(?:BOOK\\s+(?:FULL\\s+)?PROFIT(?:S)?)\\s*[:=\\-]?\\s*" + token + "\\b"),
                Pattern.compile("\\b(?:STOCK\\s*NAME|SYMBOL|TICKER|SCRIP|STOCK)\\s*[:=\\-]\\s*" + token + "\\b"),
                Pattern.compile("\\b(?:SQUARE[ -]?OFF|COST[ -]?TO[ -]?COST(?:\\s+EXIT)?)\\s*[:=\\-]?\\s*" + token + "\\b"),
                Pattern.compile("\\b(?:EXIT\\s+NOW|CLOSE\\s+(?:TRADE|POSITION))\\s*[:=\\-]?\\s*" + token + "\\b"),
